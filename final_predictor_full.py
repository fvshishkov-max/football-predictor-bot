# final_predictor_full.py
"""
Полная версия predictor.py со статистикой и XGBoost
"""
    ...
    def analyze_live_match(self, match: Match) -> Optional[Dict]:
        try:
            ...
            if hasattr(self, 'signal_validator'):
                ...
                if not valid:
                    logger.debug(f"Сигнал отклонен: {reason}")
                    return None
...
with open('predictor.py', 'w', encoding='utf-8') as f:
    f.write(final_predictor_full)

print("✅ predictor.py создан с полной функциональностью")
print("   - XGBoost инициализация")
print("   - Статистика в сигналах (удары, в створ, xG)")
print("   - Динамические пороги")
