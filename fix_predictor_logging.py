# fix_predictor_logging.py
"""
Добавляет логирование в predictor.py
Запуск: python fix_predictor_logging.py
"""
...
def fix_logging():
    ...
    # Проверяем наличие импорта logging
    ...
    # Добавляем настройку логирования в __init__
    ...
    # Добавляем запись в save_predictions
    save_method = """    def save_predictions(self, filename: str = 'data/predictions/predictions.json'):
    ...
    if match:
        new_content = content.replace(match.group(0), save_method)
        with open('predictor.py', 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("✅ Метод save_predictions обновлен с логированием")
    else:
        print("⚠️ Метод save_predictions не найден")
