# replace_predictor.py
"""
Замена predictor.py на простую рабочую версию
"""

simple_predictor = '''import numpy as np
...
class Predictor:
    def __init__(self, model_path: str = 'data/models/xgboost_model.pkl'):
        ...
        logger.info("Predictor инициализирован")
    ...
    def analyze_live_match(self, match: Match) -> Optional[Dict]:
        try:
            ...
            if prob >= self.min_signal_probability:
                ...
                logger.info(f"📢 СИГНАЛ ОТПРАВЛЕН! Матч {match.id} ({half}) - {prob*100:.1f}%")
...
print("✅ predictor.py replaced with simple working version")
